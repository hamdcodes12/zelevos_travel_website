...
async function run() {
  ...
  console.log(colors.bold("WAYORA FULL-STACK HEALTH CHECK (LOCALHOST)"));
  console.log(colors.bold("==========================================\n"));
  ...
  await check("Frontend Home page (http://localhost:3000/)", async () => {
    ...
    assert.ok(html.includes("Wayora") || html.includes("<div id=\"root\">"));
  });
  ...
  await check("Customer Signup with customerId & phone (/api/auth/signup)", async () => {
    ...
    assert.ok(setCookie && setCookie.includes("wayora_session="));
  ...
  await check("Admin Login with seeded credentials (zelevos-travelai00 / ZT002121)", async () => {
    ...
    assert.equal(res.status, 200);
    const setCookie = res.headers.get("set-cookie");
    assert.ok(setCookie && setCookie.includes("wayora_admin_session="));
