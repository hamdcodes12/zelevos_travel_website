...
export class EmailService {
  ...
  public generatePlainTextEmail(booking: Booking): string {
    ...
    return `
WAYORA FLIGHT BOOKING CONFIRMATION
==================================
...
Refund Policy: Cancellations via Wayora portal automatically calculate eligible refunds credited to original payment source.

Security Notice: All sensitive credentials, tokens, and payment secrets have been excluded.
Generated by Wayora Travel Operations.
    `.trim();
  }
...
<head>
  <meta charset="utf-8">
  <title>Wayora — Flight Booking Confirmation</title>
</head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f1f5f9; margin: 0; padding: 24px; color: #1e293b;">
  <div style="max-width: 650px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1);">
    ...
    <div style="background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%); padding: 28px 32px; color: #ffffff;">
      <h1 style="margin: 0 0 6px 0; font-size: 24px; font-weight: 800; letter-spacing: -0.5px;">WAYORA TRAVEL</h1>
    ...
    <div style="padding: 28px 32px;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 13px;">
        ...
        <tr>
          <td style="padding: 6px 0; color: #64748b;">Wayora Reference:</td>
      ...
      <div style="margin-top: 24px; padding: 14px; background-color: #f1f5f9; border-radius: 8px; font-size: 12px; color: #475569; line-height: 1.5;">
        ...
        Changes or cancellations are permitted subject to airline fare conditions up to 4 hours before scheduled departure. Real-time refunds upon eligible cancellation are credited directly to the payer's original payment method via the Wayora portal.
      </div>
    </div>
    ...
    <div style="background-color: #f8fafc; border-top: 1px solid #e2e8f0; padding: 20px 32px; font-size: 12px; color: #94a3b8; text-align: center;">
      <p style="margin: 0 0 4px 0;">This email is an automated confirmation sent by the Wayora Booking Engine.</p>
  ...
  public async sendBookingConfirmation(booking: Booking): Promise<EmailSendResult> {
    ...
    const subject = `Wayora — New Flight Booking Confirmed — ${pnrRef}`;
    ...
    if (this.config.provider === "resend" && this.config.resendApiKey) {
      try {
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: "Wayora Bookings <onboarding@resend.dev>",
            to: [recipient],
            cc: cc ? [cc] : undefined,
            subject,
            html,
    ...
    if (this.config.provider === "smtp" && this.transporter) {
      try {
        const info = await this.transporter.sendMail({
          from: `"Wayora Bookings" <${this.config.smtpUser}>`,
          to: recipient,
          cc,
          subject,
          text,
