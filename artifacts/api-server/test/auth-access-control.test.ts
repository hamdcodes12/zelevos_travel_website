...
process.env.SESSION_SECRET ??= "test-session-secret-for-wayora-auth-access-control";
process.env.PAYMENT_PROVIDER = "test";

const { default: app } = await import("../src/app");
const { db, usersTable, flightBookingsTable } = await import("@workspace/db");
...
after(async () => {
  if (createdUserEmails.length) {
    try {
      ...
      if (userIds.length) {
        await db.delete(flightBookingsTable).where(inArray(flightBookingsTable.userId, userIds));
...
describe("Wayora Authentication & Flight Booking Access Control", () => {
  ...
  describe("3. Social OAuth (Google & Facebook) Status & Handling", () => {
    it("reports configuration status via GET /api/auth/providers", async () => {
      const res = await apiRequest("/api/auth/providers");
      assert.equal(res.status, 200);
      assert.ok("google" in res.body);
      assert.ok("facebook" in res.body);
      assert.equal(typeof res.body.google.enabled, "boolean");
      assert.equal(typeof res.body.facebook.enabled, "boolean");
    });

    it("redirects with clear notice if Google OAuth is accessed unconfigured", async () => {
      const res = await apiRequest("/api/auth/google");
      assert.equal(res.status, 302);
      const location = res.headers.get("location") || "";
      assert.ok(
        location.includes("oauth_error") || location.includes("accounts.google.com"),
        `Unexpected Google OAuth redirect: ${location}`
      );
    });

    it("redirects with clear notice if Facebook OAuth is accessed unconfigured", async () => {
      const res = await apiRequest("/api/auth/facebook");
      assert.equal(res.status, 302);
      const location = res.headers.get("location") || "";
      assert.ok(
        location.includes("oauth_error") || location.includes("facebook.com"),
        `Unexpected Facebook OAuth redirect: ${location}`
      );
    });
  });

  describe("4. Flight Booking Access Control", () => {
