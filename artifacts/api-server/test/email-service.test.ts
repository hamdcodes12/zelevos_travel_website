...
describe("Email Service Unit Tests", () => {
  const mockBooking: Booking = {
    ...
    actualGrossMargin: null,
    timeline: [],
    cancellationReason: null,
    cancellationRequestedAt: null,
    itineraryUrl: null,
    customerContact: { name: "Aarav Sharma", email: "aarav.sharma@example.com", phone: "+91 98765 43210" },
    ownerId: "00000000-0000-0000-0000-000000000002",
    kind: "FLIGHT",
    status: "CONFIRMED",
    providerMode: "LIVE",
    providerReference: "SUPPLIER_REF_12345",
    bookingReference: "ZEL-XY9876",
    pnr: "XY9876",
  ...
  it("renders plain text email with all required passenger, flight, payment and booking details", () => {
    ...
    assert.match(text, /Provider Reference: SUPPLIER_REF_12345/);
  ...
  it("reports unavailable email delivery when no provider credentials exist", async () => {
    const service = new EmailService({ isTestMode: true, clientBookingEmail: "admin@zelevos.travel" });
    const res = await service.sendBookingConfirmation(mockBooking);

    assert.equal(res.success, false);
    assert.equal(res.status, "UNAVAILABLE");
    assert.equal(res.recipient, "admin@zelevos.travel");
...
describe.skip("End-to-End Flight Booking with Email Dispatch & Retry (PARKED - Phase 1: flight supplier parked)", () => {
