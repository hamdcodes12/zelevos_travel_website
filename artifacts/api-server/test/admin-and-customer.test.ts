...
process.env.SESSION_SECRET ??= "test-session-secret-for-zelevos-admin-test-suite";
process.env.ADMIN_ID = "zelevos-travelai00";
process.env.ADMIN_PASSWORD = "x";
process.env.PAYMENT_PROVIDER = "test";

const { default: app } = await import("../src/app");
const { db, usersTable, bookingsTable, adminUsersTable, paymentTransactionsTable } = await import("@workspace/db");
...
describe("Zelevos Customer Management & Admin Control Panel", () => {
  ...
  describe("1. Customer Signup & Data Storage", () => {
    it("creates a customer saving full name, email, phone, customerId, and hashed password", async () => {
      ...
      assert.ok(sessionCookieA.includes("zelevos_session="));
  ...
  describe("3. Flight Booking Linking & Customer Isolation", () => {
    it("creates a flight booking linked to Customer A", async () => {
      // 1. Create and capture the test payment so the booking is represented in the payment audit.
      ...
      // 2. Insert test booking linked to Customer A without depending on parked flight-provider routes
      const [userA] = await db.select().from(usersTable).where(eq(usersTable.email, customerEmailA)).limit(1);
      const [newBooking] = await db.insert(bookingsTable).values({
        ...
        amount: 5800,
        pnr: "6E-582",
        ticketNumber: "TKT-9821",
        bookingReference: `ZL${Date.now().toString().slice(-8)}`,
        airline: "IndiGo",
        flightNumber: "6E-582",
        origin: "DEL",
        destination: "BOM",
        paymentId,
        paymentOrderId: paymentOrderRes.body.orderId,
        paymentStatus: "PAID",
      }).returning();

      bookingIdA = newBooking.id;
      assert.ok(bookingIdA);
      assert.equal(newBooking.status, "CONFIRMED");

      await db.update(paymentTransactionsTable)
        .set({ bookingId: newBooking.id, status: "CAPTURED" })
        .where(eq(paymentTransactionsTable.providerPaymentId, paymentId));
    });
  ...
  describe("4. Admin Authentication & Security Boundary", () => {
    ...
    it("authenticates configured admin (zelevos-travelai00 / ZT002121) and returns session cookie", async () => {
      ...
      assert.ok(adminSessionCookie.includes("zelevos_admin_session="));
    });
