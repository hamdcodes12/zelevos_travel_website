...
async function request<T = Record<string, any>>(path: string, init?: RequestInit): Promise<{ response: Response; body: T }> {
  const response = await originalFetch(`${baseUrl}${path}`, init);
  const body = (await response.json()) as T;
  return { response, body };
}

// PARKED (Phase 1, 2026-09-19): AI Travel OS tests skipped per Wayora_PRD_Without_APIs.
describe.skip("AI API contract (PARKED - Phase 1 per Wayora_PRD_Without_APIs)", () => {
  it("returns the expected health response structure", async () => {
    mockGemini("ZELEVOS_GEMINI_OK");
