...
import { syncHotelGuestRooms, toHotelbedsGuestRooms, validateHotelGuestRooms } from "../src/lib/hotel-guests";

describe("Hotelbeds per-pax guest form", () => {
  ...
  it("creates multiple rooms with children and preserves each exact age", () => {
    ...
    const payload = toHotelbedsGuestRooms(rooms);
  ...
  it("rejects missing names and accepts a complete normalized payload", () => {
    ...
    assert.deepEqual(toHotelbedsGuestRooms(rooms)[0].paxes[1], { roomId: 1, name: "Child", surname: "Guest", type: "CH", age: 7 });
  });
});
